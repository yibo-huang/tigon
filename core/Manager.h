...
class Manager : public Worker {
    public:
	Manager(std::size_t coordinator_id, std::size_t id, const Context &context, std::atomic<bool> &stopFlag)
	...
	{
		if (coordinator_id == 0) {
			LOG(INFO) << "Manager(worker id = " << id << ") on the coordinator node started.";
			coordinator_start();
			LOG(INFO) << "Manager(worker id = " << id << ") on the coordinator node exits.";
		} else {
			LOG(INFO) << "Manager(worker id = " << id << ") on the non-coordinator node started.";
			non_coordinator_start();
			LOG(INFO) << "Manager(worker id = " << id << ") on the non-coordinator node exits.";
		}
	}
	...
	void push_message(Message *message) override
	{
		...
		auto message_type = static_cast<ControlMessage>(messagePiece.get_message_type());
