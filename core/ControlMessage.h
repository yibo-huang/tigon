...
class ControlMessageFactory {
	...
	static std::size_t new_statistics_message(Message &message, int coordinator_id, double commit,
                uint64_t size_index_usage, uint64_t size_metadata_usage, uint64_t size_data_usgae, uint64_t size_transport_usage)
	{
		/*
		...
		auto message_size = MessagePiece::get_header_size() + sizeof(coordinator_id) + sizeof(commit) +
                        sizeof(size_index_usage) + sizeof(size_metadata_usage) + sizeof(size_data_usgae) + sizeof(size_transport_usage);
		...
		encoder << coordinator_id << commit << size_index_usage << size_metadata_usage << size_data_usgae << size_transport_usage;
		message.flush();
