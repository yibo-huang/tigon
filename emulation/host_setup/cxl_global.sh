...
disable_ht()
{
    cur_ht=$(cat /sys/devices/system/cpu/smt/control)
    if [ $cur_ht = "notsupported" ]; then
        echo "can't control hyperthreading"
    else
...
set_performance_mode()
{
    ...
    if [ -f "/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor" ]; then
        ...
        echo "can't control scaling governor"
    fi
}

disable_node1_mem()
{
    echo 0 | sudo tee /sys/devices/system/node/node1/memory*/online >/dev/null 2>&1 || true
}
...
check_pmqos()
{
    ...
    sudo nohup ${CXL_SCRIPT_DIR}/pmqos >/dev/null 2>&1 &
    sleep 3
    # double check
...
configure_cxl_exp_cores()
{
    ...
    { echo 0 | sudo tee /sys/devices/system/node/node1/cpu*/online >/dev/null 2>&1; } || true
}
...
configure_base_exp_cores()
{
    ...
    for ((i = half_cores_per_socket; i < cores_per_socket; i++)); do
        { echo 0 | sudo tee /sys/devices/system/cpu/cpu$i/online >/dev/null 2>&1; } || true
    done
...
check_base_conf()
{
    ...
    disable_thp
    # disable_ht
    enable_ht  # enable hyperthreading
    disable_turbo
    configure_base_exp_cores
    check_pmqos
    disable_swap
...
check_comm_conf() {
    ...
    # bring_all_cpus_online
    sudo modprobe msr
    if cat /proc/cpuinfo | grep -i "vendor_id" | grep -i "intel"; then
        cd $CXL_SCRIPT_DIR/../bandwidth_latency
        sudo python3 uncore_freq.py set --freq 2400 --socket 0
        sudo python3 uncore_freq.py set --freq 0 --socket 1
        sudo python3 uncore_freq.py print
        cd $CXL_SCRIPT_DIR
    fi
}
