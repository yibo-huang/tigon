...
template <typename T> class TrivialKey {
    public:
	TrivialKey() = default;
	TrivialKey(T v)
		: value(v)
...
using btree_t = btreeolc_cxl::BPlusTree<KeyType, Value, Comparator<KeyType>, Comparator<Value>, 10, leaf_size, inner_size>;

template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void singleInsertTest(btree_t<KeyType, leaf_size, inner_size> &btree)
{
	std::vector<int> keys;
	...
	std::cout << "inner nodes " << inner_nodes << ", leaf nodes " << leaf_nodes << ", kv_size " << kv_size << ", tree_bytes " << tree_bytes << ", # keys "
		  << 100000 << ", # keys per leave " << 100000.0 / leaf_nodes << std::endl;
	;
}
...
template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void singleDeleteTest(btree_t<KeyType, leaf_size, inner_size> &btree)
{
	std::vector<int> keys;
	constexpr int num_keys = 100000;
	for (int i = 0; i < num_keys; ++i) {
	...
	auto leaf_nodes = btree.getNumLeafNodes();
	auto tree_bytes = leaf_size * leaf_nodes + inner_nodes * inner_size;
	std::cout << "After deletes, inner nodes " << inner_nodes << ", leaf nodes " << leaf_nodes << ", kv_size " << kv_size << ", tree_bytes " << tree_bytes
		  << std::endl;
	;
}

template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void singleGetValueTest(btree_t<KeyType, leaf_size, inner_size> &btree)
{
	for (int i = 0; i < 100000; i++) {
		KeyType k{ i };
...
template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void singleScanTest(btree_t<KeyType, leaf_size, inner_size> &btree)
{
...
template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void singleScanForUpdateTest(btree_t<KeyType, leaf_size, inner_size> &btree)
{
	{
		KeyType l{ 0 }, h{ 50000 };
		int vSeq = 0;
...
template <typename KeyType, uint64_t leaf_size, uint64_t inner_size> void multiScanDelInsert1Test(btree_t<KeyType, leaf_size, inner_size> &btree)
{
	for (int i = 0; i < 100000; i++) {
		KeyType k{ i };
		bool res = btree.remove(k);
...
template <typename T, uint64_t leaf_size, uint64_t inner_size> void multiScanDelInsert2Test(btree_t<T, leaf_size, inner_size> &btree)
{
	for (int i = 0; i < 100000; i++) {
		btree.remove(i * 2);
	}
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_QueryDelete()
{
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_QueryInsert()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_DeleteDelete()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_DeleteDeletePredicate()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
		...
		for (int i = 0; i < 4; ++i) {
			delete_threads.emplace_back(
				[&btree](int id) {
					...
					for (int i = 0; i < kElementCount; ++i) {
						KeyType k{ i };
						btree.remove(k, [](const Value &) -> btreeolc_cxl::RemovePredicateResult {
							return btreeolc_cxl::RemovePredicateResult::GOOD;
						});
						delete[] k.transfer();
					}
					// std::cout << "thread #" << id << " finish delete\n";
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_InsertInsert()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_InsertDeleteScanForUpdate()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
	...
	for (int i = 0; i < kIterNum; ++i) {
		...
		std::cout << "After deletes, inner nodes " << inner_nodes << ", leaf nodes " << leaf_nodes << ", kv_size " << kv_size << ", tree_bytes "
			  << tree_bytes << std::endl;
		;
	}
	std::cout << '\n';
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_InsertDelete()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
...
template <typename KeyType, uint64_t page_size = 4096> void ConcurrentTest_InsertDeleteQuery()
{
	// using Type = SharedPodWrapper<long>;
	btree_t<KeyType, page_size, page_size> btree;
