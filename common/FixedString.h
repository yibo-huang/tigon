...
template <std::size_t N> class FixedString {
	...
	std::size_t hash_code() const
	{
		...
		for (auto i = 0u; i < N; i++) {
			hashCode = star::hash_combine(hashCode, h(data_[i]));
		}
		return hashCode;
	}
...
template <class C, std::size_t N> inline std::basic_ostream<C> &operator<<(std::basic_ostream<C> &os, const FixedString<N> &str)
{
	os << str.toString();
	return os;
}
...
namespace std
{
template <std::size_t N> struct hash<star::FixedString<N> > {
	std::size_t operator()(const star::FixedString<N> &k) const
	{
		return k.hash_code();
	}
};
