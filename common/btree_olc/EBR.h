...
template <std::size_t UpdateThreshold, class Deallocator> class EBR {
    private:
	EBR()
	{
	}
	...
	struct ThreadData {
		~ThreadData()
		{
			...
			if (std::any_of(retire_lists_.begin(), retire_lists_.end(), [](std::list<void *> p) { return p.size() != 0; })) {
				...
				auto target_epoch = (global_epoch_.load(std::memory_order_relaxed) + kNumberEpochs - 1) % kNumberEpochs;
				...
				global_thread_block_list_.abandon_retired_nodes(new Orphan<kNumberEpochs>(target_epoch, retire_lists_));
			}
		...
		void doEnterCritical()
		{
			...
			if (control_block_->local_epoch_.load(std::memory_order_relaxed) != epoch) {
				...
				if (!tryUpdateEpoch(epoch, new_epoch))
					return;
				epoch = new_epoch;
			} else {
		...
		bool tryUpdateEpoch(unsigned curr_epoch, unsigned new_epoch)
		{
			...
			auto can_update = !std::any_of(global_thread_block_list_.begin(), global_thread_block_list_.end(), prevents_update);
			if (!can_update)
				return false;
			...
			if (global_epoch_.load(std::memory_order_relaxed) == curr_epoch) {
				...
				bool success =
					global_epoch_.compare_exchange_strong(curr_epoch, new_epoch, std::memory_order_release, std::memory_order_relaxed);
		...
		void adoptOrphans()
		{
			Orphan<kNumberEpochs> *cur = global_thread_block_list_.adopt_abandoned_retired_nodes();
	...
	template <class T> class ThreadBlockList {
	    public:
		~ThreadBlockList()
		{
		...
		ThreadControlBlock *acquire_entry()
		{
		...
		void abandon_retired_nodes(Orphan<kNumberEpochs> *obj)
		{
			...
			do {
				last->next_ = h;
			} while (!abandoned_retired_nodes_.compare_exchange_weak(h, obj, std::memory_order_release, std::memory_order_relaxed));
		}

		Orphan<kNumberEpochs> *adopt_abandoned_retired_nodes()
		{
			if (abandoned_retired_nodes_.load(std::memory_order_relaxed) == nullptr)
				return nullptr;
		...
		void add_entry(T *node)
		{
			...
			do {
				node->next_entry_ = h;
			} while (!head_.compare_exchange_weak(h, node, std::memory_order_release, std::memory_order_relaxed));
		}
...
template <std::size_t UpdateThreshold, class Deallocator> std::atomic<unsigned> EBR<UpdateThreshold, Deallocator>::global_epoch_;
