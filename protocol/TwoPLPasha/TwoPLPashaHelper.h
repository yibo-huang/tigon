...
class TwoPLPashaHelper {
	...
	uint64_t read(const std::tuple<MetaDataType *, void *> &row, void *dest, std::size_t size, std::atomic<uint64_t> &local_cxl_access)
	{
		        ...
		        std::memcpy(dest, src, size);
                        ...
                        smeta->lock();
                        CHECK(scc_data->get_flag(TwoPLPashaMetadataShared::valid_flag_index) == true);
        ...
        void update(const std::tuple<MetaDataType *, void *> &row, const void *value, std::size_t value_size)
	{
		...
		lmeta->lock();
                ...
                if (lmeta->is_migrated == false) {
                        ...
                        TwoPLPashaMetadataShared *smeta = reinterpret_cast<TwoPLPashaMetadataShared *>(lmeta->migrated_row);
                        TwoPLPashaMetadataSharedSCC *scc_data = smeta->scc_data.get();
                        void *data_ptr = smeta->scc_data->data;
	...
	static uint64_t read_lock(std::atomic<uint64_t> &meta, bool &success)
	{
                ...
                if (lmeta->is_migrated == false) {
                        ...
                        if (scc_data->get_flag(TwoPLPashaMetadataShared::valid_flag_index) == false) {
                                smeta->unlock();
                                success = false;
                                goto out_unlock_lmeta;
                        }
        ...
        static void modify_tuple_valid_bit(std::atomic<uint64_t> &meta, bool is_valid)
        {
                ...
                if (lmeta->is_migrated == true) {
                        ...
                        if (is_valid == true) {
                                ...
                                scc_data->clear_flag(TwoPLPashaMetadataShared::valid_flag_index);
                        }
                        smeta->unlock();
                } else {
                ...
                auto move_in_processor = [&](const void *prev_key, void *prev_meta, void *prev_data, const void *cur_key, void *cur_meta, void *cur_data, const void *next_key, void *next_meta, void *next_data) {
                        ...
                        if (cur_lmeta->is_migrated == false) {
                                ...
                                if (cur_lmeta->is_valid == true) {
                                        cur_scc_data->set_flag(TwoPLPashaMetadataShared::valid_flag_index);
                                } else {
                                        cur_scc_data->clear_flag(TwoPLPashaMetadataShared::valid_flag_index);
                                }
                                ...
                                if (is_next_key_migrated == true) {
                                        cur_scc_data->set_flag(TwoPLPashaMetadataShared::is_next_key_real_flag_index);
                                } else {
                                        cur_scc_data->clear_flag(TwoPLPashaMetadataShared::is_next_key_real_flag_index);
                                }

                                // update the prev-key information
                                if (is_prev_key_migrated == true) {
                                        cur_scc_data->set_flag(TwoPLPashaMetadataShared::is_prev_key_real_flag_index);
                                } else {
                                        cur_scc_data->clear_flag(TwoPLPashaMetadataShared::is_prev_key_real_flag_index);
                                }
                                ...
                                if (prev_lmeta != nullptr) {
                                        ...
                                        if (prev_lmeta->is_migrated == true) {
                                                ...
                                                auto prev_scc_data = reinterpret_cast<TwoPLPashaMetadataSharedSCC *>(prev_smeta->scc_data.get());

                                                prev_smeta->lock();
                                                prev_scc_data->set_flag(TwoPLPashaMetadataShared::is_next_key_real_flag_index);
                                                prev_smeta->unlock();
                                        }
                                        prev_lmeta->unlock();
                                }
                                ...
                                if (next_lmeta != nullptr) {
                                        ...
                                        if (next_lmeta->is_migrated == true) {
                                                ...
                                                auto next_scc_data = reinterpret_cast<TwoPLPashaMetadataSharedSCC *>(next_smeta->scc_data.get());

                                                next_smeta->lock();
                                                next_scc_data->set_flag(TwoPLPashaMetadataShared::is_prev_key_real_flag_index);
                                                next_smeta->unlock();
                                        }
                                        next_lmeta->unlock();
                                }
        ...
        bool move_from_hashmap_to_partition(ITable *table, const void *key, const std::tuple<MetaDataType *, void *> &row)
	{
		...
		TwoPLPashaMetadataLocal *lmeta = reinterpret_cast<TwoPLPashaMetadataLocal *>(meta.load());
                ...
                if (lmeta->is_migrated == true) {
                        ...
                        // copy metadata back
                        lmeta->is_valid = scc_data->get_flag(TwoPLPashaMetadataShared::valid_flag_index);
                ...
                auto move_out_processor = [&](const void *prev_key, void *prev_meta, void *prev_data, const void *cur_key, void *cur_meta, void *cur_data, const void *next_key, void *next_meta, void *next_data) {
                        ...
                        if (prev_lmeta != nullptr) {
                                ...
                                if (prev_lmeta->is_migrated == true) {
                                        ...
                                        prev_smeta->lock();
                                        prev_scc_data->clear_flag(TwoPLPashaMetadataShared::is_next_key_real_flag_index);
                                        prev_smeta->unlock();
                                }
                                prev_lmeta->unlock();
                        }
                        ...
                        if (next_lmeta != nullptr) {
                                ...
                                if (next_lmeta->is_migrated == true) {
                                        ...
                                        next_smeta->lock();
                                        next_scc_data->clear_flag(TwoPLPashaMetadataShared::is_prev_key_real_flag_index);
                                        next_smeta->unlock();
                                }
                                next_lmeta->unlock();
                        }
                        ...
                        if (cur_lmeta->is_migrated == true) {
                                ...
                                // copy metadata back
                                cur_lmeta->is_valid = cur_scc_data->get_flag(TwoPLPashaMetadataShared::valid_flag_index);
        ...
        bool delete_and_update_next_key_info(ITable *table, const void *key, bool is_local_delete, bool &need_move_out_from_migration_tracker, void *&migration_policy_meta)
	{
                 auto adjacent_tuples_processor = [&](const void *prev_key, void *prev_meta, void *prev_data, const void *cur_key, void *cur_meta, void *cur_data, const void *next_key, void *next_meta, void *next_data) -> bool {
                        ...
                        if (cur_lmeta->is_migrated == true) {
                                ...
                                auto cur_scc_data = cur_smeta->scc_data.get();

                                cur_smeta->lock();
                                if (is_local_delete == true) {
                                        CHECK(cur_scc_data->get_flag(TwoPLPashaMetadataShared::valid_flag_index) == true);
                                        cur_scc_data->clear_flag(TwoPLPashaMetadataShared::valid_flag_index);
                                } else {
