		...
		TwoPLPashaRWKey &scanKey = txn->scanSet[key_offset];
                ...
                auto remote_scan_processor = [&](const void *key, void *cxl_row, bool is_last_tuple) -> bool {
                        ...
                        if (table.compare_key(key, min_key) == 0) {
                                ...
                                if (smeta->get_flag(TwoPLPashaMetadataShared::is_next_key_real_flag_index) == false || smeta->get_flag(TwoPLPashaMetadataShared::is_prev_key_real_flag_index) == false) {
                                        migration_required = true;
                                }
                        }
                        smeta->unlock();
