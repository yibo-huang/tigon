...
template <class Database> class TwoPLGC {
	...
	TwoPLGC(DatabaseType &db, const ContextType &context, Partitioner &partitioner)
		: db(db)
		, context(context)
		...
		for (auto i = 0u; i < readSet.size(); i++) {
			...
			if (readKey.get_read_lock_bit()) {
				if (partitioner.has_master_partition(partitionId)) {
					...
					txn.network_size +=
						MessageFactoryType::new_abort_message(*syncMessages[coordinatorID], *table, readKey.get_key(), false);
				}
			}
			...
			if (readKey.get_write_lock_bit()) {
				if (partitioner.has_master_partition(partitionId)) {
					...
					auto coordinatorID = partitioner.master_coordinator(partitionId);
					txn.network_size +=
						MessageFactoryType::new_abort_message(*syncMessages[coordinatorID], *table, readKey.get_key(), true);
				}
			}
		}
	...
	void write(TransactionType &txn, uint64_t commit_tid, std::vector<std::unique_ptr<Message> > &messages)
	{
		auto &readSet = txn.readSet;
		auto &writeSet = txn.writeSet;
		...
		for (auto i = 0u; i < writeSet.size(); i++) {
			...
			if (partitioner.has_master_partition(partitionId)) {
				...
				auto coordinatorID = partitioner.master_coordinator(partitionId);
				txn.network_size +=
					MessageFactoryType::new_write_message(*messages[coordinatorID], *table, writeKey.get_key(), writeKey.get_value());
			}
		}
	...
	void replicate(TransactionType &txn, uint64_t commit_tid, std::vector<std::unique_ptr<Message> > &messages)
	{
		auto &readSet = txn.readSet;
		auto &writeSet = txn.writeSet;
