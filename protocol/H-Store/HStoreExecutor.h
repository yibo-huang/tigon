...
{
	switch (stage) {
	case ExecutorStage::NORMAL:
		return "NORMAL";
	case ExecutorStage::CHECKPOINT_START:
		return "CHECKPOINT_START";
	case ExecutorStage::CHECKPOINT_COW:
		return "CHECKPOINT_COW";
	case ExecutorStage::CHECKPOINT_COW_DUMP_FINISHED:
		return "CHECKPOINT_COW_DUMP_FINISHED";
	case ExecutorStage::CHECKPOINT_DONE:
...
class WindowAverage {
    public:
	WindowAverage(std::size_t window_size)
...
class HStoreExecutor : public Executor<Workload, HStore<typename Workload::DatabaseType> >

{
	...
	const std::string memory_usage_from_stats()
	{
		...
		ss << "txns_replayed " << txns_replayed << ", delete_mp_txns " << delete_mp_txns << "\n"
		   ...
		   << "txns_candidate_for_reexecution " << txns_candidate_for_reexecution.size() << " sizeof(TransactionType) " << sizeof(TransactionType)
		   << ", total bytes " << (txns_candidate_for_reexecution.size() * sizeof(TransactionType)) << "\n"
	...
	struct Lock {
		...
		{
			return "[write-locked=" + std::to_string(write_locked()) + ", readers=" + std::to_string(reader_cnt()) +
			       ", last-writer=" + std::to_string(get_last_writer()) + "]";
		}
		...
		void set_reader_cnt(int64_t readers)
		{
			...
			lock_word = get_word_with_reader_cnt_and_sign_cleared() | (((uint64_t)readers) << kReaderCntShift) | (signbit << kReaderCntSignShift);
		}
	...
	HStoreExecutor(std::size_t coordinator_id, std::size_t worker_id, DatabaseType &db, const ContextType &context, std::atomic<uint32_t> &worker_status,
		...
		, window_persistence_latency(5000)
		, window_txn_latency(5000)
		, window_queued_lock_req_latency(5000)
		, window_lock_req_latency(5000)
		, window_active_mps(5000)
		, lock_bm(this->context.partition_num * this->context.granules_per_partition)
	{
		...
		if (is_replica_worker == false) {
			hash_partitioner = PartitionerFactory::create_partitioner("hash", coordinator_id, this->partitioner->num_coordinator_for_one_replica());
				...
				if (this_cluster_worker_id == partition_owner_cluster_worker(p, 0)) {
					...
					for (std::size_t j = 0; j < this->context.granules_per_partition; ++j) {
						...
						if (replica_num > 1) {
							...
							DCHECK(replica_cluster_worker_id == -1 ||
							       standby_replica_cluster_worker_id == replica_cluster_worker_id);
		...
		LOG(INFO) << "Cluster worker id " << this_cluster_worker_id << " node worker id " << worker_id << " partitions managed ["
			  << managed_granules_str << "], replica granules maanged [" << managed_replica_granules_str << "]"
	...
	void search(std::size_t table_id, std::size_t partition_id, const void *key, void *value) const
	{
	...
	void abort(TransactionType &txn, std::vector<std::unique_ptr<Message> > &messages, bool write_cmd_buffer = false)
	{
		...
		if (!txn.is_single_partition()) {
			...
			for (auto i = 0u; i < txn.lock_status.num_locks(); ++i) {
				...
				if (owner_cluster_worker == this_cluster_worker_id) {
					...
					DCHECK(0 <= owner_cluster_worker && owner_cluster_worker < cluster_worker_num);
					uint64_t requested_last_writer = lock.get_last_writer(); // Restore the last writer with last txn that sucessfully
												 // committed while write-locked the granule
					DCHECK(txn.transaction_id != requested_last_writer);
					DCHECK(write_cmd_buffer == false);
					txn.network_size += MessageFactoryType::new_release_lock_message(*messages[owner_cluster_worker], *table,
													 this_cluster_worker_id, granule_id,
													 requested_last_writer, write_lock, false,
													 txn.ith_replica, write_cmd_buffer, txn_cmd);
					// LOG(INFO) << "Abort release lock MP partition " << partition_id << " by cluster worker" << this_cluster_worker_id <<
					// " " << tid_to_string(txn.transaction_id) << " request sent";
			...
			for (auto i = 0u; i < txn.lock_status.num_locks(); ++i) {
				...
				if (lock.get_mode() == TransactionType::LockStatus::LockMode::READ) {
					...
					DCHECK(lock_bucket.get_last_writer() == txn.transaction_id);
					DCHECK(txn.transaction_id != lock.get_last_writer());
					lock_bucket.set_last_writer(lock.get_last_writer()); // Restore the last writer with last txn that sucessfully committed
											     // while write-locked the granule
					lock_bucket.clear_write_lock();
				}
				lock.set_released();
			}
	...
	{
		ScopedTimer t0([&, this](uint64_t us) { serialize_cmd_time.add(us); });
		...
		command_buffer_outgoing_data.insert(command_buffer_outgoing_data.end(), command_buffer_data.begin() + command_buffer_data_size,
						    command_buffer_data.end());
	...
	bool commit_transaction(TransactionType &txn, std::vector<std::unique_ptr<Message> > &messages)
	{
	...
	void send_participant_command(TransactionType &txn, std::vector<std::unique_ptr<Message> > &messages)
	{
		...
		for (auto i = 0u; i < txn.lock_status.num_locks(); ++i) {
			...
			if (owner_cluster_worker != this_cluster_worker_id) {
				...
				txn.network_size += MessageFactoryType::new_write_participant_command_message(*messages[owner_cluster_worker], *table,
													      this_cluster_worker_id, granule_id,
													      requested_last_writer, write_lock,
	...
	void write_back_command_logging(TransactionType &txn, uint64_t commit_tid, std::vector<std::unique_ptr<Message> > &messages)
	{
		...
		for (auto i = 0u; i < writeSet.size(); i++) {
			...
			if ((int)owner_cluster_worker == this_cluster_worker_id) {
				...
				DCHECK(0 <= owner_cluster_worker && owner_cluster_worker < cluster_worker_num);
				messages[owner_cluster_worker]->set_transaction_id(txn.transaction_id);
				txn.network_size += MessageFactoryType::new_write_back_message(*messages[owner_cluster_worker], *table, writeKey.get_key(),
											       writeKey.get_value(), this_cluster_worker_id,
											       writeKey.get_granule_id(), commit_tid, txn.ith_replica, false);
	...
	void setupHandlers(TransactionType &txn)
		...
		override
	{
		txn.lock_request_handler = [this, &txn](std::size_t table_id, std::size_t partition_id, std::size_t granule_id, uint32_t key_offset,
							const void *key, void *value, bool local_index_read, bool write_lock, bool &success, bool &remote) {
			auto transaction_id = txn.transaction_id;
			if (is_replica_worker == true && sp_parallel_exec_commit && txn.is_single_partition() &&
			    txn.replay_bm_for_sp->get_bit(to_lock_id(partition_id, granule_id))) {
				success = false;
				remote = false;
			...
			if ((int)owner_cluster_worker == this_cluster_worker_id) {
				...
				if (is_replica_worker) {
					...
					if (success_state == TransactionType::LockStatus::SuccessState::INIT) { // fisrt lock attempt of this transaction on
														// this granule
				...
				//    LOG(INFO) << "Tranasction from worker " << this_cluster_worker_id << " locked partition " << lock_id << " txn " <<
				//    tid_to_string(txn.transaction_id);;
				// if (lock_bucket == -1) {
				...
				// LOG(INFO) << "Requesting locking partition " << partition_id << " by cluster worker" << this_cluster_worker_id << " on
				// owner_cluster_worker " << owner_cluster_worker;
				DCHECK(0 <= owner_cluster_worker && owner_cluster_worker < cluster_worker_num);
				...
				if (is_replica_worker && write_lock) {
					...
					granule_last_writer = txn.transaction_id; // for write lock request, the last successful writer is the transaction
										  // itself.
				}
				txn.network_size += MessageFactoryType::new_acquire_lock_and_read_message(*(cluster_worker_messages[owner_cluster_worker]),
													  *table, key, key_offset, this_cluster_worker_id,
													  granule_id, granule_last_writer, write_lock,
													  request_lock, txn.ith_replica, 0);
				add_outgoing_message(owner_cluster_worker);
	...
	int cnt = 0;
	bool acquire_lock_and_read_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						   Transaction *txn)
	{
		++cnt;
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::ACQUIRE_LOCK_AND_READ_REQUEST));
		...
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + key_size + sizeof(key_offset) + sizeof(requested_last_writer) +
								  sizeof(write_lock) + sizeof(request_lock) + sizeof(uint32_t) + sizeof(uint32_t) +
		...
		if (is_replica_worker) {
			...
			//       //lock it;
			//       // if (lock_bucket == -1)
			//       //    LOG(INFO) << "Partition " << lock_id << " locked and read by remote cluster worker " << request_remote_worker_id << " by
			//       this_cluster_worker_id " << this_cluster_worker_id << " ith_replica "  << ith_replica << " txn " << tid_to_string(tid);
			//       lock_bucket = tid;
			//       lock_bm.set_bit(lock_id);
			//       success = 1;
			//     } else {
			//       //  LOG(INFO) << "Partition " << lock_id << " was failed to be locked by cluster worker " << request_remote_worker_id << " and
			//       txn " << tid_to_string(tid)
		...
		auto message_size = MessagePiece::get_header_size() + sizeof(success) + sizeof(key_offset) + sizeof(lock_bucket_last_writer) +
				    sizeof(write_lock) + sizeof(request_lock) + sizeof(uint64_t) * 3;

		if (success == 1) {
			// window_lock_req_latency.add(std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()).time_since_epoch().count()
			// - inputMessage.get_message_recv_time()); this->last_window_lock_req_latency.store(window_lock_req_latency.average());
		...
		auto message_piece_header = MessagePiece::construct_message_piece_header(static_cast<uint32_t>(HStoreMessage::ACQUIRE_LOCK_AND_READ_RESPONSE),
											 message_size, table_id, partition_id, granule_id);
	...
	struct ReplayBuffer {
		std::string data;
		Decoder *dec = nullptr;
		ReplayBuffer(const std::string &buf)
			: data(buf)
			, dec(new Decoder(data))
		{
		}
		ReplayBuffer(const std::string &&buf)
			: data(std::move(buf))
			...
			for (int32_t j = 0; j < partition_count; ++j) {
				...
				if (partition_responsible_worker == this_cluster_worker_id) {
					for (int k = 0; k < partition_granule_count; ++k) {
						...
						cmd_mp.write_lock = mp_txn->lock_status.get_lock(lock_index).get_mode() ==
								    TransactionType::LockStatus::LockMode::WRITE;
						...
						if (cmd_mp.write_lock) {
							...
							if (lock_bucket.write_locked() || lock_bucket.get_last_writer() != cmd_mp.last_writer) {
								...
								// LOG(INFO) << "This cluster worker " << this_cluster_worker_id << " replayed participant read
								// lock record of txn " << cmd.tid << " on lock_id " << lock_id << " with last writer " <<
								// cmd.last_writer;
								DCHECK(get_partition_last_replayed_position_in_log(lock_id) <= cmd_mp.position_in_log);
								get_partition_last_replayed_position_in_log(lock_id) = cmd_mp.position_in_log;
							}
			...
			if (cmd.is_cow_begin || cmd.is_cow_end) {
				...
				if (cmd.write_lock) {
					...
					if (lock_bucket.write_locked() || lock_bucket.get_last_writer() != cmd.last_writer) {
						...
						lock_bucket.inc_reader_cnt();
						// LOG(INFO) << "This cluster worker " << this_cluster_worker_id << " replayed participant read lock record of
						// txn " << cmd.tid << " on lock_id " << lock_id << " with last writer " << cmd.last_writer;
						DCHECK(get_partition_last_replayed_position_in_log(lock_id) <= cmd.position_in_log);
						get_partition_last_replayed_position_in_log(lock_id) = cmd.position_in_log;
					}
				}
	...
	void command_replication_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						 Transaction *txn)
	{
		DCHECK(is_replica_worker);
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::COMMAND_REPLICATION_REQUEST));
		...
		ScopedTimer t([&, this](uint64_t us) { spread_time.add(us); });
	...
	void command_replication_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						  Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::COMMAND_REPLICATION_RESPONSE));
		DCHECK(is_replica_worker == false);
		txn->pendingResponses--;
		txn->network_size += inputPiece.get_message_length();
	}

	void acquire_lock_and_read_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						    Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::ACQUIRE_LOCK_AND_READ_RESPONSE));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
		...
		if (success == 1 && (lstatus.get_success() == TransactionType::LockStatus::SuccessState::REQUESTED ||
				     lstatus.get_success() == TransactionType::LockStatus::SuccessState::SUCCEED)) {
			uint32_t msg_length = inputPiece.get_message_length();
			auto header_size = MessagePiece::get_header_size();
			uint32_t exp_length = header_size + sizeof(success) + sizeof(key_offset) + sizeof(last_writer) + sizeof(write_lock) +
					      sizeof(requested_lock) + sizeof(uint64_t) * 3 + value_size;
			...
			// DCHECK(inputPiece.get_message_length() ==
			//        MessagePiece::get_header_size() + sizeof(last_writer) + sizeof(write_lock) + sizeof(requested_lock) + sizeof(uint64_t) * 3 +
			//        sizeof(success) +
			//            sizeof(key_offset));
			if (success == 2) {
			...
			txn->abort_lock = true;
			DCHECK(lstatus.get_success() == TransactionType::LockStatus::SuccessState::REQUESTED ||
			       lstatus.get_success() == TransactionType::LockStatus::SuccessState::FAILED);
		...
		if (txn->lock_request_responded == false) {
			...
			// txn->first_lock_request_arrive_latency = ts2 -
			// std::chrono::time_point_cast<std::chrono::microseconds>(txn->lock_issue_time).time_since_epoch().count();
			// txn->first_lock_request_processed_latency = ts3 -
			// std::chrono::time_point_cast<std::chrono::microseconds>(txn->lock_issue_time).time_since_epoch().count();
	...
	void write_back_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::WRITE_BACK_REQUEST));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
		...
		if (success) {
			...
			DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(ith_replica) + sizeof(granule_id) +
									  sizeof(commit_tid) + sizeof(persist_commit_record) + key_size + field_size +
									  sizeof(uint32_t));
			const void *key = stringPiece.data();
			stringPiece.remove_prefix(key_size);
		...
		auto message_piece_header = MessagePiece::construct_message_piece_header(static_cast<uint32_t>(HStoreMessage::WRITE_BACK_RESPONSE),
											 message_size, table_id, partition_id, granule_id);
	...
	void write_back_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::WRITE_BACK_RESPONSE));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
		...
		DCHECK(success);
		if (success) {
			DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(success));
		} else {
			DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(success));
			txn->abort_lock = true;
		}
	...
	static std::size_t new_get_replayed_log_posistion_response_message(Message &responseMessage, int64_t replayed_posisiton)
	{
		...
		auto message_piece_header = MessagePiece::construct_message_piece_header(
			static_cast<uint32_t>(HStoreMessage::GET_REPLAYED_LOG_POSITION_RESPONSE), message_size, 0, 0);
	...
	void rtt_test_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::RTT_REQUEST));
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(int) + sizeof(int));
		...
		auto message_piece_header =
			MessagePiece::construct_message_piece_header(static_cast<uint32_t>(HStoreMessage::RTT_RESPONSE), message_size, 0, 0);
	...
	void get_replayed_log_position_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						       Transaction *txn)
	{
		ScopedTimer t([&, this](uint64_t us) { replay_query_time.add(us); });
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::GET_REPLAYED_LOG_POSITION_REQUEST));
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(int) + sizeof(int) + sizeof(int64_t));
		DCHECK(is_replica_worker);
		int cluster_worker_id, ith_replica;
		auto stringPiece = inputPiece.toStringPiece();
		Decoder dec(stringPiece);
		...
		// LOG(INFO) << "cluster_worker " << cluster_worker_id << " called persist_cmd_buffer_request_handler on cluster worker " <<
		// this_cluster_worker_id;
	...
	void get_replayed_log_position_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
							Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::GET_REPLAYED_LOG_POSITION_RESPONSE));
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(int64_t));
		DCHECK(is_replica_worker == false);
		auto stringPiece = inputPiece.toStringPiece();
	...
	void persist_cmd_buffer_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::PERSIST_CMD_BUFFER_REQUEST));
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(int) + sizeof(int));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		...
		auto message_piece_header = MessagePiece::construct_message_piece_header(static_cast<uint32_t>(HStoreMessage::PERSIST_CMD_BUFFER_RESPONSE),
	...
	void persist_cmd_buffer_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						 Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::PERSIST_CMD_BUFFER_RESPONSE));
		DCHECK(inputPiece.get_message_length() == MessagePiece::get_header_size() + sizeof(int));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		auto stringPiece = inputPiece.toStringPiece();
		...
		// LOG(INFO) << "cluster_worker " << this_cluster_worker_id << " received response from persist_cmd_buffer_request_handler on cluster worker "
		// << cluster_worker_id << ", received_persist_cmd_buffer_responses " << received_persist_cmd_buffer_responses;
	}

	void write_participant_command_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table,
						       Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::WRITE_PARTICIPANT_COMMAND_REQUEST));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
	...
	void release_lock_request_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::RELEASE_LOCK_REQUEST));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
		...
		// } else {
		//   // if (lock_bucket != -1)
		//   //   LOG(INFO) << "Partition " << lock_id_to_string(lock_id) << " unlocked by cluster worker" << request_remote_worker << " by
		//   this_cluster_worker_id " << this_cluster_worker_id << " ith_replica " << ith_replica << " txn " << tid_to_string(tid);
		//   DCHECK(lock_bm.get_bit(lock_id) == true);
		//   lock_bucket = -1;
		...
		// prepare response message header
		auto message_size = MessagePiece::get_header_size() + sizeof(bool);
		auto message_piece_header = MessagePiece::construct_message_piece_header(static_cast<uint32_t>(HStoreMessage::RELEASE_LOCK_RESPONSE),
											 message_size, table_id, partition_id, granule_id);
	...
	void release_lock_response_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::RELEASE_LOCK_RESPONSE));
		auto table_id = inputPiece.get_table_id();
		auto partition_id = inputPiece.get_partition_id();
		DCHECK(table_id == table.tableID());
		DCHECK(partition_id == table.partitionID());
	...
	bool process_single_transaction(TransactionType *txn, bool lock_acquired = false)
	{
		...
		auto ltc = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - txn->startTime).count();
		...
		if (result == TransactionResult::READY_TO_COMMIT) {
			...
			if (commit) {
				...
				commit_interval.add(
					std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - last_commit).count());
				last_commit = std::chrono::steady_clock::now();
				return true;
			} else {
				//        DCHECK(txn->is_single_partition() == false);
				// if (is_replica_worker)
				//   LOG(INFO) << "Txn " << txn_id << " Execution result " << (int)result << " abort by lock conflict on cluster worker " <<
				//   this_cluster_worker_id;
	...
	std::size_t process_to_commit(
		std::deque<TransactionType *> &to_commit, std::function<void(TransactionType *)> post_commit = [](TransactionType *) {})
	{
		std::size_t cnt = 0;
	...
	void process_single_txn_commit(TransactionType *txn)
	{
		...
		bool commit;
		{
			ScopedTimer t([&, this](uint64_t us) {
				if (commit) {
					...
					auto ltc =
						std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - txn->startTime).count();
					txn->set_stall_time(ltc);
				}
			});
			commit = this->commit_transaction(*txn, cluster_worker_messages);
		...
		if (commit) {
			...
			active_txns.erase(txn->transaction_id);
			commit_interval.add(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - last_commit).count());
			last_commit = std::chrono::steady_clock::now();
		} else {
			DCHECK(false);
			// if (is_replica_worker)
	...
	void execute_transaction_batch_haa(const std::vector<TransactionType *> all_txns, const std::vector<TransactionType *> &sp_txns,
					   std::vector<TransactionType *> &mp_txns)
	{
		...
		{
			ScopedTimer t0([&, this](uint64_t us) { execution_phase_time.add(us); });
			...
			if (is_replica_worker == false && replica_num > 1) {
				...
				while (minimum_replayed_log_position < minimum_coord_txn_written_log_position_snap) {
					...
					ScopedTimer t2([&, this](uint64_t us) { this->replica_progress_query_latency.add(us); });
					// Keep querying the replica for its replayed log position until minimum_replayed_log_position >=
					// minimum_coord_txn_written_log_position_snap
					DCHECK(0 <= replica_cluster_worker_id && replica_cluster_worker_id < cluster_worker_num);
					cluster_worker_messages[replica_cluster_worker_id]->set_transaction_id(txn_id);
					MessageFactoryType::new_get_replayed_log_posistion_message(*cluster_worker_messages[replica_cluster_worker_id],
												   minimum_coord_txn_written_log_position_snap, 1,
												   this_cluster_worker_id);
					add_outgoing_message(replica_cluster_worker_id);
					flush_messages();
		...
		ScopedTimer t0([&, this](uint64_t us) { execution_after_commit_time.add(us); });
		{
			ScopedTimer t0([&, this](uint64_t us) { commit_persistence_us += us; });
		...
		for (size_t i = 0; i < txns.size(); ++i) {
			...
			auto latency = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - txn->startTime).count();
	...
	void execute_transaction_batch(const std::vector<TransactionType *> all_txns, const std::vector<TransactionType *> &sp_txns,
				       std::vector<TransactionType *> &mp_txns)
	{
		last_commit = std::chrono::steady_clock::now();
		std::vector<bool> workers_need_persist_cmd_buffer(this->active_replica_worker_num_end, false);
	...
	void process_batch_of_transactions()
	{
		...
		std::vector<TransactionType *> mp_txns;
		{
			ScopedTimer t0([&, this](uint64_t us) { scheduling_time += us; });
			for (size_t i = 0; i < txns.size(); ++i) {
	...
	void process_new_transactions()
	{
		scheduling_time = 0;
		{
			ScopedTimer t([&, this](uint64_t us) { scheduling_time += us; });
		...
		while (q.empty() == false) {
			...
			if (cmd.is_cow_end || cmd.is_cow_begin) {
				...
				if (((ReplicaCheckpointMeta *)cmd.txn)->ref_cnt == 0) {
					std::string tag = cmd.is_cow_begin ? "COW_BEGIN" : "COW_END";
					LOG(INFO) << "Replica worker " << this_cluster_worker_id << " " << tag << " at step "
						  << ((ReplicaCheckpointMeta *)cmd.txn)->checkpoint_step << " started";
					if (tag == "COW_BEGIN") {
					...
					for (auto lock_id : lock_ids_by_this_replica_executor) {
						...
						DCHECK(get_granule_lock_cmd_queue(lock_id).front().is_cow_begin ||
						       get_granule_lock_cmd_queue(lock_id).front().is_cow_end);
					...
					LOG(INFO) << "Replica worker " << this_cluster_worker_id << " " << tag << " at step "
						  << ((ReplicaCheckpointMeta *)cmd.txn)->checkpoint_step << " finished";
				}
				break;
				...
				if (cmd.write_lock) {
					if (lock_bucket.free()) { // only acquire the lock when there is no readers and writer
						...
						DCHECK(get_partition_last_replayed_position_in_log(lock_id) <= cmd.position_in_log);
						get_partition_last_replayed_position_in_log(lock_id) = cmd.position_in_log;
						get_granule_lock_time(lock_id) = std::chrono::duration_cast<std::chrono::microseconds>(
					...
					// LOG(INFO) << "This cluster worker " << this_cluster_worker_id << " replayed participant read lock record of txn " <<
					// cmd.tid << " on lock_id " << lock_id << " with last writer " << cmd.last_writer;
					DCHECK(get_partition_last_replayed_position_in_log(lock_id) <= cmd.position_in_log);
					get_partition_last_replayed_position_in_log(lock_id) = cmd.position_in_log;
					get_granule_lock_time(lock_id) =
	...
	void replay_loop()
	{
		...
		auto complete_mp = [&, this](TransactionType *mp_txn) {
			...
			if (mp_txn->finished_commit_phase && (mp_txn->abort_lock == false || mp_txn->abort_no_retry)) {
				...
				auto latency =
					std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - mp_txn->startTime).count();
				this->percentile.add(latency);
				window_txn_latency.add(latency);
				this->last_window_txn_latency.store(window_txn_latency.average());
				this->dist_latency.add(latency);
	...
	void send_commands_to_replica(bool persist = false)
	{
		...
		DCHECK(0 <= replica_cluster_worker_id && replica_cluster_worker_id < cluster_worker_num);
		MessageFactoryType::new_command_replication(*cluster_worker_messages[replica_cluster_worker_id], 1, command_buffer_outgoing_data,
							    this_cluster_worker_id, persist);
		add_outgoing_message(replica_cluster_worker_id);
		flush_messages();
		// LOG(INFO) << "This cluster worker " << this_cluster_worker_id << " sent " << command_buffer_outgoing.size() << " commands to replia worker "
		// <<  replica_cluster_worker_id;
	...
	void process_queue_partition_lock_request(int lock_id)
	{
		...
		for (std::size_t i = 0; i < q.size(); ++i) {
			...
			ITable *table = this->db.find_table(messagePiece.get_table_id(), messagePiece.get_partition_id());
			TransactionType *txn = nullptr;
			bool res = acquire_lock_and_read_request_handler(*message, messagePiece,
									 *cluster_worker_messages[message->get_source_cluster_worker_id()], *table, txn);
			if (res == true) {
				window_queued_lock_req_latency.add(
					std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()).time_since_epoch().count() -
					message->get_message_recv_time());
	...
	std::size_t handle_requests(bool should_replay_commands = true)
	{
		...
		while (!this->in_queue.empty()) {
...

			{
				...
				//     msg_proc_latency.add(std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()).time_since_epoch().count()
				//     - message->get_message_gen_time());
				//     //msg_recv_latency.add(std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()).time_since_epoch().count()
				//     - message->get_message_recv_time());
				//   }
				// });
				...
				for (auto it = message->begin(); it != message->end(); it++, ++msg_idx) {
					...
					// if (type != (int)HStoreMessage::MASTER_UNLOCK_PARTITION_RESPONSE && type !=
					// (int)HStoreMessage::MASTER_LOCK_PARTITION_RESPONSE
					//     && type != (int)HStoreMessage::ACQUIRE_PARTITION_LOCK_AND_READ_RESPONSE && type != (int)
					//     HStoreMessage::WRITE_BACK_RESPONSE && type != (int)HStoreMessage::RELEASE_READ_LOCK_RESPONSE
					//     && type != (int)HStoreMessage::RELEASE_PARTITION_LOCK_RESPONSE && type != (int)HStoreMessage::PREPARE_REQUEST &&
					//     type != (int)HStoreMessage::PREPARE_RESPONSE && type != (int)HStoreMessage::PREPARE_REDO_REQUEST && type !=
					//     (int)HStoreMessage::PREPARE_REDO_RESPONSE) {
					...
					if (type == (int)HStoreMessage::ACQUIRE_LOCK_AND_READ_REQUEST) {
						bool res = acquire_lock_and_read_request_handler(
							*message, messagePiece, *cluster_worker_messages[message->get_source_cluster_worker_id()], *table, txn);
						if (res == false) {
							if (is_replica_worker) {
						...
						acquire_lock_and_read_response_handler(
							*message, messagePiece, *cluster_worker_messages[message->get_source_cluster_worker_id()], *table, txn);
						DCHECK(txn);
						if (txn->pendingResponses == 0) {
							async_txns_to_commit.push_back(txn);
							if (txn->abort_lock == false) {
								...
								// std::chrono::steady_clock::now() - txn->lock_issue_time)
								// .count();
								// tries_lock_response_latency[std::min((int)txn->tries, (int)tries_lock_response_latency.size()
								// - 1)].add(ltc);
							}
						}
	...
	{
		if (this->context.lotus_checkpoint == LotusCheckpointScheme::COW_ON_CHECKPOINT_OFF_LOGGING_ON ||
		    this->context.lotus_checkpoint == COW_OFF_CHECKPOINT_OFF_LOGGING_ON)
			return false;
	...
	void checkpoint_inst_handler(const Message &inputMessage, MessagePiece inputPiece, Message &responseMessage, ITable &table, Transaction *txn)
	{
		DCHECK(inputPiece.get_message_type() == static_cast<uint32_t>(HStoreMessage::CHECKPOINT_INST));
	...
	enum class CheckpointCowOperation { BEGIN, END };
		...
		while (checkpoint_insts.empty() == false) {
			...
			if (this_cluster_worker_id == 0) { // checkpoint coordinator
				if (stage == ExecutorStage::CHECKPOINT_START) {
					...
					LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  " << this->this_cluster_worker_id
						  << " received a CHECKPOINT_START_RESP from " << inst.source_cluster_id << ", checkpointPendingResponses "
						  << checkpointPendingResponses;
					if (--checkpointPendingResponses == 0) {
						LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  swtich to "
							  << to_string(ExecutorStage::CHECKPOINT_COW);
						...
						CHECK((int)this->context.worker_num * (int)this->partitioner->num_coordinator_for_one_replica() ==
						      checkpointPendingResponses);
					}
					...
					CHECK(checkpointPendingResponses > 0);
					LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  " << this->this_cluster_worker_id
						  << " received a CHECKPOINT_COW_DUMP_FINISHED from " << inst.source_cluster_id
						  << ", checkpointPendingResponses " << checkpointPendingResponses;
					if (--checkpointPendingResponses == 0) { // All workers finish dumping data
						LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  swtich to "
							  << to_string(ExecutorStage::CHECKPOINT_DONE);
						stage = ExecutorStage::CHECKPOINT_DONE;
						send_to_checkpoint_participants(CheckpointAction::CHECKPOINT_DONE_REQUEST, true);
						CHECK((int)this->context.worker_num * (int)this->partitioner->num_coordinator_for_one_replica() - 1 ==
						      checkpointPendingResponses);
					}
					...
					CHECK(checkpointPendingResponses > 0);
					LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  " << this->this_cluster_worker_id
						  << " received a CHECKPOINT_DONE_RESP from " << inst.source_cluster_id << ", checkpointPendingResponses "
						  << checkpointPendingResponses;
					if (--checkpointPendingResponses == 0) { // All workers synchornized, switch to normal mode
						LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage) << "]  swtich to "
							  << to_string(ExecutorStage::NORMAL);
				...
				if (stage == ExecutorStage::NORMAL) {
					...
					LOG(INFO) << "Checkpoint participant worker[stage=" << to_string(stage) << "]  swtich to "
						  << to_string(ExecutorStage::CHECKPOINT_START);
					...
					CHECK(action == CheckpointAction::CHECKPOINT_COW_DUMP_FINISHED);
					LOG(INFO) << "Checkpoint participant worker[stage=" << to_string(stage) << "]  swtich to "
						  << to_string(ExecutorStage::CHECKPOINT_COW_DUMP_FINISHED);
			...
			if (stage == ExecutorStage::NORMAL) {
				...
				if (this->db.checkpoint_work_finished(managed_partitions)) {
					if (this_cluster_worker_id == 0) {
						LOG(INFO) << "Checkpoint coordinator worker[stage=" << to_string(stage)
							  << "]  finished COW dump, switching to CHECKPOINT_COW_DUMP_FINISHED stage";
						stage = ExecutorStage::CHECKPOINT_COW_DUMP_FINISHED;
					}
	...
	void start() override
	{
		...
		while ((status = static_cast<ExecutorStatus>(this->worker_status.load())) != ExecutorStatus::START) {
		...
		while (static_cast<ExecutorStatus>(this->worker_status.load()) != ExecutorStatus::CLEANUP) {
	...
	void onExit() override
	{
		...
		//     message_processing_latency_str += "Message type " + std::to_string(i) + " count " + std::to_string(message_processing_latency[i].size())
		//     + " avg " + std::to_string(message_processing_latency[i].avg()) + " 50th " + std::to_string(message_processing_latency[i].nth(50)) + "
		//     75th " + std::to_string(message_processing_latency[i].nth(75)) + " 95th " + std::to_string(message_processing_latency[i].nth(95)) + "\n";
		//   }
		// }
		// std::string tries_latency_str = "\ntries_latency_str\n";
		// for (size_t i = 0; i < tries_latency.size(); ++i) {
		//   if (tries_latency[i].size() > 0) {
		//     tries_latency_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_latency[i].size()) + " avg " +
		//     std::to_string(tries_latency[i].avg()) + " 50th " + std::to_string(tries_latency[i].nth(50)) + " 75th " +
		//     std::to_string(tries_latency[i].nth(75)) + " 95th " + std::to_string(tries_latency[i].nth(95)) +  " 99th " +
		//     std::to_string(tries_latency[i].nth(99)) +  " 100th " + std::to_string(tries_latency[i].nth(100)) +"\n";
		//   }
		// }
		// std::string tries_prepare_time_str = "\ntries_prepare_time_str\n";
		// for (size_t i = 0; i < tries_latency.size(); ++i) {
		//   if (tries_prepare_time[i].size() > 0) {
		//     tries_prepare_time_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_prepare_time[i].size()) + " avg " +
		//     std::to_string(tries_prepare_time[i].avg()) + " 50th " + std::to_string(tries_prepare_time[i].nth(50)) + " 75th " +
		//     std::to_string(tries_prepare_time[i].nth(75)) + " 95th " + std::to_string(tries_prepare_time[i].nth(95)) + "\n";
		//   }
		// }
		// std::string tries_lock_stall_time_str = "\ntries_lock_stall_time_str\n";
		// for (size_t i = 0; i < tries_latency.size(); ++i) {
		//   if (tries_lock_stall_time[i].size() > 0) {
		//     tries_lock_stall_time_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_lock_stall_time[i].size()) + " avg " +
		//     std::to_string(tries_lock_stall_time[i].avg()) + " 50th " + std::to_string(tries_lock_stall_time[i].nth(50)) + " 75th " +
		//     std::to_string(tries_lock_stall_time[i].nth(75)) + " 95th " + std::to_string(tries_lock_stall_time[i].nth(95)) + "\n";
		//   }
		// }
		// std::string tries_execution_done_time_str = "\ntries_execution_done_time_str\n";
		// for (size_t i = 0; i < tries_latency.size(); ++i) {
		//   if (tries_execution_done_latency[i].size() > 0) {
		//     tries_execution_done_time_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_execution_done_latency[i].size()) + "
		//     avg " + std::to_string(tries_execution_done_latency[i].avg()) + " 50th " + std::to_string(tries_execution_done_latency[i].nth(50)) + "
		//     75th " + std::to_string(tries_execution_done_latency[i].nth(75)) + " 95th " + std::to_string(tries_execution_done_latency[i].nth(95)) +
		//     "\n";
		//   }
		// }
		...
		//     tries_first_lock_response_latency_str += "Tries " + std::to_string(i) + " count " +
		//     std::to_string(tries_first_lock_response_latency[i].size()) + " avg " + std::to_string(tries_first_lock_response_latency[i].avg()) + "
		//     50th " + std::to_string(tries_first_lock_response_latency[i].nth(50)) + " 75th " +
		//     std::to_string(tries_first_lock_response_latency[i].nth(75)) + " 95th " + std::to_string(tries_first_lock_response_latency[i].nth(95)) +
		//     "\n";
		//   }
		// }
		...
		//     tries_first_lock_request_arrive_latency_str += "Tries " + std::to_string(i) + " count " +
		//     std::to_string(tries_first_lock_request_arrive_latency[i].size()) + " avg " +
		//     std::to_string(tries_first_lock_request_arrive_latency[i].avg()) + " 50th " +
		//     std::to_string(tries_first_lock_request_arrive_latency[i].nth(50)) + " 75th " +
		//     std::to_string(tries_first_lock_request_arrive_latency[i].nth(75)) + " 95th " +
		//     std::to_string(tries_first_lock_request_arrive_latency[i].nth(95)) + "\n";
		//   }
		// }
		...
		//     tries_first_lock_request_processed_latency_str += "Tries " + std::to_string(i) + " count " +
		//     std::to_string(tries_first_lock_request_processed_latency[i].size()) + " avg " +
		//     std::to_string(tries_first_lock_request_processed_latency[i].avg()) + " 50th " +
		//     std::to_string(tries_first_lock_request_processed_latency[i].nth(50)) + " 75th " +
		//     std::to_string(tries_first_lock_request_processed_latency[i].nth(75)) + " 95th " +
		//     std::to_string(tries_first_lock_request_processed_latency[i].nth(95)) + "\n";
		//   }
		// }
		// std::string tries_lock_response_latency_str = "\ntries_last_lock_response_latency_str\n";
		// for (size_t i = 0; i < tries_latency.size(); ++i) {
		//   if (tries_lock_response_latency[i].size() > 0) {
		//     tries_lock_response_latency_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_lock_response_latency[i].size()) + "
		//     avg " + std::to_string(tries_lock_response_latency[i].avg()) + " 50th " + std::to_string(tries_lock_response_latency[i].nth(50)) + " 75th
		//     " + std::to_string(tries_lock_response_latency[i].nth(75)) + " 95th " + std::to_string(tries_lock_response_latency[i].nth(95)) + "\n";
		//   }
		// }
		...
		//     tries_commit_initiated_latency_str += "Tries " + std::to_string(i) + " count " + std::to_string(tries_commit_initiated_latency[i].size())
		//     + " avg " + std::to_string(tries_commit_initiated_latency[i].avg()) + " 50th " +
		//     std::to_string(tries_commit_initiated_latency[i].nth(50)) + " 75th " + std::to_string(tries_commit_initiated_latency[i].nth(75)) + " 95th
		//     " + std::to_string(tries_commit_initiated_latency[i].nth(95)) + "\n";
		//   }
		// }
		...
		LOG(INFO) << (is_replica_worker ? "Replica" : "") << " Worker " << this->id << " commit: " << this->worker_commit
			  ...
			  << ". effective batch concurrency: " << this->effective_round_concurrency.nth(50) << ". latency: " << this->percentile.nth(50)
			  << " us (50%) " << this->percentile.nth(75) << " us (75%) " << this->percentile.nth(95) << " us (95%) " << this->percentile.nth(99)
			  << " us (99%)  " << this->percentile.avg() << " us (avg). "
			  << "dist txn latency: " << this->dist_latency.nth(50) << " us (50%) " << this->dist_latency.nth(75) << " us (75%) "
			  << this->dist_latency.nth(95) << " us (95%) " << this->dist_latency.nth(99) << " us (99%) "
			  << " avg " << this->dist_latency.avg() << " us (avg). "
			  << " local txn latency: " << this->local_latency.nth(50) << " us (50%) " << this->local_latency.nth(75) << " us (75%) "
			  << this->local_latency.nth(95) << " us (95%) " << this->local_latency.nth(99)
			  << " us (99%). txn try times : " << this->txn_try_times.nth(50) << " (50%) " << this->txn_try_times.nth(75) << " (75%) "
			  << this->txn_try_times.nth(95) << " (95%) " << this->txn_try_times.nth(99) << " (99%). "
			  << " spread_avg_time " << spread_time.avg() << " spread cnt " << spread_cnt.avg() << " replay_query_time " << replay_query_time.avg()
			  << ". "
			  << " replica progress query latency " << replica_progress_query_latency.nth(50) << " us(50%), "
			  << replica_progress_query_latency.avg() << " us(avg). "
			  << " replication gap " << replication_gap_after_active_replica_execution.avg() << " replication sync comm rounds "
			  << replication_sync_comm_rounds.avg() << " replication time " << replication_time.avg() << " txn tries " << txn_retries.nth(50)
			  << " 50th " << txn_retries.nth(75) << " 75th " << txn_retries.nth(95) << " 95th " << txn_retries.nth(99) << " 99th "
			  << txn_retries.nth(100) << " 100th "
			  << " acquire lock ltc " << acquire_lock_message_latency.nth(50) << " 50th " << acquire_lock_message_latency.nth(75) << " 75th "
			  << acquire_lock_message_latency.nth(95) << " 95th "
			  << " single_partition_mp " << single_worker_mp_txn << " cross_worker_mp " << cross_worker_mp_txn << " cross_node_mp "
			  << cross_node_mp_txn << " replay_loop_time " << replay_loop_time.nth(50) << " " << replay_loop_time.nth(75) << " "
			  << replay_loop_time.nth(95) << " " << replay_loop_time.avg() << " replay_concurrency " << replay_mp_concurrency.nth(50) << " "
			  << replay_mp_concurrency.nth(75) << " " << replay_mp_concurrency.nth(95) << " " << replay_mp_concurrency.avg()
			  << " round_mp_initiated " << round_mp_initiated.nth(50) << " " << round_mp_initiated.nth(75) << " " << round_mp_initiated.nth(95)
			  << " zero_mp_initiated_cost " << zero_mp_initiated_cost.avg() << " commit interval " << commit_interval.nth(50) << " "
			  << commit_interval.nth(75) << " " << commit_interval.nth(95) << " " << commit_interval.nth(99) << " " << commit_interval.avg()
			  << " mp_arrival_interval " << mp_arrival_interval.nth(50) << " " << mp_arrival_interval.nth(75) << " " << mp_arrival_interval.nth(95)
			  << " " << mp_arrival_interval.avg() << " handle request latency " << handle_latency.nth(50) << " " << handle_latency.nth(75) << " "
			  << handle_latency.nth(95) << " " << handle_latency.avg() << " handle message count " << handle_msg_cnt.nth(50) << " "
			  << handle_msg_cnt.nth(75) << " " << handle_msg_cnt.nth(95) << " " << handle_msg_cnt.avg() << " rtt " << rtt_stat.size() << " "
			  << rtt_stat.nth(50) << " " << rtt_stat.nth(75) << " " << rtt_stat.nth(95) << " " << rtt_stat.avg() << " msg_send_latency "
			  << msg_send_latency.nth(50) << " " << msg_send_latency.nth(75) << " " << msg_send_latency.nth(95) << " " << msg_send_latency.avg()
			  << " msg_recv_latency " << msg_recv_latency.nth(50) << " " << msg_recv_latency.nth(75) << " " << msg_recv_latency.nth(95) << " "
			  << msg_recv_latency.avg() << " msg_proc_latency " << msg_proc_latency.nth(50) << " " << msg_proc_latency.nth(75) << " "
			  << msg_proc_latency.nth(95) << " "
			  << msg_proc_latency.avg()
			  ...
			  << " mp_concurrency_limit " << mp_concurrency_limit.avg() << " mp_avg_abort_rate " << mp_avg_abort.avg() << " cmd queue time "
			  << cmd_queue_time.nth(50) << " cmd stall time by lock " << cmd_stall_time.nth(50) << " serialize_cmd_time "
			  << serialize_cmd_time.avg() << " execution phase time " << execution_phase_time.avg() << " execution_after_commit_time "
			  << execution_after_commit_time.avg() << " scheduling_time " << scheduling_cost.avg() << ". \n"
			  ...
			  << " commit_prepare " << this->dist_txn_commit_prepare_time_pct.avg() << " us " << this->dist_txn_commit_prepare_time_pct.avg()
			  << " us, "
			  ...
			  << this->dist_txn_commit_write_back_time_pct.avg() << " us, "
		...
		if (this->id == 0) {
			for (auto i = 0u; i < this->message_stats.size(); i++) {
				LOG(INFO) << "message stats, type: " << i << " count: " << this->message_stats[i] << " total size: " << this->message_sizes[i];
			}
	...
	int partition_owner_worker_id_on_a_node(int partition_id) const
	{
		...
		auto node_worker_id_this_partition_belongs_to = nth_partition_on_master_coord % this->context.worker_num; // A worker could handle more than 1
															  // partition
