...
class AriaHelper {
	...
	static void set_key_tid(AriaRWKey &key, const std::tuple<std::atomic<uint64_t> *, void *> &row)
	{
		key.set_tid(std::get<0>(row));
	}

	static std::atomic<uint64_t> &get_metadata(ITable *table, const AriaRWKey &key)
	{
	...
	static bool reserve_read(std::atomic<uint64_t> &a, uint64_t epoch, uint32_t tid)
	{
	...
	static bool reserve_write(std::atomic<uint64_t> &a, uint64_t epoch, uint32_t tid)
	{
		uint64_t old_value, new_value;
		do {
			old_value = a.load();
