...
template <class Workload> class AriaManager : public star::Manager {
	...
	ContextType ctx;
	AriaManager(std::size_t coordinator_id, std::size_t id, DatabaseType &db, const ContextType &context, std::atomic<bool> &stopFlag)
		...
		, workload(coordinator_id, db, random, *this->partitioner)
		, ctx(context)
		, db(db)
		, epoch(0)
	{
	...
	void log_transactions()
	{
		...
		{
			ScopedTimer t2([&, this](uint64_t us) { commit_persistence_time = us; });
