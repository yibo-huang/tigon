...
class makeNewOrderQuery {
    public:
	NewOrderQuery operator()(const Context &context, int32_t W_ID, Random &random) const
	{
		NewOrderQuery query;
		...
		for (auto i = 0; i < query.O_OL_CNT; i++) {
			...
			do {
				retry = false;
				query.INFO[i].OL_I_ID = random.non_uniform_distribution(8191, 1, 100000);
				for (int k = 0; k < i; k++) {
			...
			if (i == 0) {
				...
				if (((warmed_up == false && context.newOrderCrossPartitionProbability > 0) || x <= context.newOrderCrossPartitionProbability) && context.partition_num > 1) {
...
class makePaymentQuery {
    public:
	PaymentQuery operator()(const Context &context, int32_t W_ID, Random &random) const
	{
		PaymentQuery query;
		...
		int x = random.uniform_dist(1, 100);
                ...
                // during warm up, we always do remote transactions to quickily saturate CXL memory
		if (((warmed_up == false && context.paymentCrossPartitionProbability > 0) || x <= context.paymentCrossPartitionProbability) && context.partition_num > 1) {
