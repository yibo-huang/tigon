...
typeset PASHA_CXL_TRANS_ENTRY_STRUCT_SIZE=256
        ...
        if [ $PROTOCOL = "SundialPasha" ]; then
                ...
                do
                        ssh_command "cd pasha; nohup ./bench_tpcc --logtostderr=1 --id=$i --servers=\"$SERVER_STRING\"
                                ...
                                --use_cxl_transport=$USE_CXL_TRANS --cxl_trans_entry_struct_size=$PASHA_CXL_TRANS_ENTRY_STRUCT_SIZE --cxl_trans_entry_num=$CXL_TRANS_ENTRY_NUM
                ...
                do
                        ssh_command "cd pasha; nohup ./bench_tpcc --logtostderr=1 --id=$i --servers=\"$SERVER_STRING\"
                                ...
                                --partitioner=hash --hstore_command_logging=false
                                --replica_group=1 --lock_manager=0 --batch_flush=1 --lotus_async_repl=true --batch_size=0 --time_to_run=$TIME_TO_RUN --time_to_warmup=$TIME_TO_WARMUP
                                --use_cxl_transport=$USE_CXL_TRANS --cxl_trans_entry_struct_size=$BASELINE_CXL_TRANS_ENTRY_STRUCT_SIZE --cxl_trans_entry_num=$CXL_TRANS_ENTRY_NUM
