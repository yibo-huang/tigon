...
def get_row(input):
        ...
        for line in fileinput.FileInput(input[1]):
                tokens = line.strip().split()
                if len(tokens) > 3 and tokens[3] == "Dispatcher.h:153]":
                        tputs.append(tokens[9])
        # local_CXL_access_ratio, remote_access_with_req_ratio, Move_in, Move_out
        for line in fileinput.FileInput(input[1]):
                tokens = line.strip().split()
                if len(tokens) > 3 and tokens[3] == "Coordinator.h:275]":
                        tputs.append(tokens[30].split(",")[0].replace("(", "").replace(")", ""))
                        tputs.append(tokens[35].split(",")[0].replace("(", "").replace(")", ""))
                        tputs.append(tokens[37].split(",")[0])
                        tputs.append(tokens[39])
        return tputs
...
def parse_tpcc(res_dir):
        ...
        header_row = ["Policy", "Tput", "MsgExchange", "local_CXL_access_ratio", "remote_access_with_req_ratio", "Move_in", "Move_out"]
